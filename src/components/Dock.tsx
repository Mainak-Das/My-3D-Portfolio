...
export default function Dock() {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 z-50 flex h-14 md:h-16 items-end gap-2 md:gap-4 rounded-2xl border border-white/10 bg-white/5 px-2 md:px-4 pb-3 backdrop-blur-md"
    >
      {DOCK_ITEMS.map((item) => (
...
function DockIcon({
  ...
  // Since we are server/client mixed, straight window access might be tricky during hydrate.
  // But `useTransform` runs on client.
  // let's simplify: make base size smaller for everyone? No, desktop needs to be big.
  // We can use a small hack or a proper hook.
  // Let's rely on the container constraining it? No, motion style `width` overrides CSS.

  // Let's check width on mount
  ...
  const widthSync = useTransform(distance, [-range, 0, range], [baseWidth, maxWidth, baseWidth]);
  ...
  return (
    <Link href={item.href} target={item.external ? "_blank" : undefined}>
      <motion.div
        ref={ref}
        style={{ width, height: width }} // Sync height with width for perfect circle
        className="rounded-full bg-gray-700/50 border border-white/10 flex items-center justify-center hover:bg-gray-600/80 transition-colors group relative"
      >
